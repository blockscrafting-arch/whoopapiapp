...
7. Запуск:
   ...
   ```
8. Откройте `http://localhost:8000`. В `.env` для локалки можно `DEBUG=true` — сессионная cookie работает без HTTPS. Для реального OAuth с WHOOP нужен публичный **HTTPS** (ngrok или VPS); на проде держите `DEBUG=false`.
...
- **Safari / iPhone**: на проде сессия OAuth настроена под редирект с WHOOP (`SameSite=None`, Secure). Подробности и чеклист деплоя — [AGENTS.md](AGENTS.md).
